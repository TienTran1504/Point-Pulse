...
import { checkCurrentDate, getFifteenDaysAfterDate, getFifteenDaysAgoDate, getWeeksInRange } from '~/utils/handleDate';
...
function Personal() {
  ...
  const before15Days = getFifteenDaysAgoDate()
  const after15Days = getFifteenDaysAfterDate()
  ...
  const [fromDate, setFromDate] = useState(before15Days);
  const [toDate, setToDate] = useState(after15Days);
  ...
  const fetchPersonalProject = async () => {
    try {
      const response = await request.get(`point_management/project_users/get_personal`, {
        headers: headers,
      });
  ...
  const fetchWeekPoints = async () => {
    ...
    for (let i = 0; i < newWeeksInfo.length; i++) {
      ...
      try {
        const response = await request.post(`point_management/get_personal_week_points_by_week`, { time: time[0] }, {
          headers: headers,
        });
  ...
  const handleProjectData = () => {
    if (weekPoints !== undefined) {
      ...
      const filteredNewData = newData.filter((item) => item.users.length > 0);
      setProjectData(filteredNewData)
      setSelectedProjects(filteredNewData)
  ...
  //     }
  //     try {
  //       await request.post(`point_management/insert_plan_point`, params, {
  //         headers: headers,
  //       });
  //     } catch (error) {
  //       console.error("Handling Data Error: ", error.message)
  ...
  const renderWeek = (formattedWeek) => {
        ...
        checkPermission('5') ? (
          ...
          <div>
            ...
            {projectUsers.length > 0 ? (
                    ...
                    <table className={classes["table-project-point"]}>
                      <thead className={`${classes.stickyRow}`}>
                        <tr >
                          ...
                          <th className={`${classes.stickyRow}`} colSpan={weeksInfo.length}>
                            <div className={`${classes.elementHeaderFirst}`}>{user.name}</div>
                          </th>
                        ...
                        <tr>
                          ...
                          {weeksInfo.map((week, index) => (
                            <th className={`${classes.stickyRow}`} key={`${week.formattedWeek}-${index}`} >
                              <div className={`${classes.elementHeaderFirst}`}>{renderWeek(week.formattedWeek)}</div>
                            </th>
                          ))}
                              ...
                              weeksInfo.map((week) => (
                                <td className={`${classes.element}`} key={`${user.id}-${week.formattedWeek}`}>
                                  <input
                                    ...
                                    disabled={checkCurrentDate(week.formattedWeek)}
                                  />
              ...
              <>
                <h2>User Haven't Joined Into Any Projects</h2>
              </>
            )}
